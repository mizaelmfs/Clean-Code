...
class LoginUserUseCaseTest {
    ...
    private fun setupUseCase(
         getUserByEmailUseCase: GetUserByEmailUseCase =
            mock(GetUserByEmailUseCase::class.java),
         addLoggedInEmailToDatastoreUseCase: AddLoggedInEmailToDatastoreUseCase =
            mock(AddLoggedInEmailToDatastoreUseCase::class.java)
    ) : LoginUserUseCase {
    ...
    fun passwordDoNotMatch() {
        val email = "email"
        val dpPassword = "user's actual password"
        val methodInputPassword = "password"

        val useCase = setupUseCase(
            getUserByEmailUseCase = mock(GetUserByEmailUseCase::class.java) {
                UserDTO(0, email, dpPassword)
            }
        )
